...
public class ReactiveAgent extends Agent {
	private Point lastRockCord;
	private static Board board;
	private CognitiveAgent carrier;
	...
	public CognitiveAgent getCarrier() {
		return carrier;
	}
	...
	private void sendCoord(Point coords) {
		carrier.getRockCoordsQueue().add(new Point(coords.x, coords.y));
	}
	...
	void moveToBase() {
		...
		if (foundValidMove) {
			...
			this.setCurrPos(this.getNewPos());
			if (board.m[this.getCurrPos().x][this.getCurrPos().y] == "B") {
				System.out.println("Agent " + this.getName() + " has found base.");
			} else 
				board.m[this.getCurrPos().x][this.getCurrPos().y] = this.getName();
		}
	}
